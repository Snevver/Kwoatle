  ...
  const deleteQuote = async () => {
    ...
    try {
      ...
      if (storedCategories) {
        ...
        const categoryIndex = categories.findIndex((cat) => cat.id === foundQuote.categoryId);

        if (categoryIndex !== -1 && categories[categoryIndex].amountOfQuotes > 0) {
      ...
      setQuotes(allQuotes.filter((quote) => quote.categoryId === Number(categoryId)));
    } catch (error) {
    ...
    <View style={[styles.quoteCard, { backgroundColor: category?.color || '#76DAE5' }]}>
      ...
      <View style={styles.quoteDetails}>
        ...
        <Text style={[globalStyles.text, styles.quoteDate]}>
          Added: {item.dateAdded ? formatDate(item.dateAdded) : 'Unknown date'}
        </Text>
      </View>
